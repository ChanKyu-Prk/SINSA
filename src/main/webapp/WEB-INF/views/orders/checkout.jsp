					...
					<table id="itemList" class="mx-auto px-0">
						<thead>
							<span class="tableHead row mx-auto px-0 mb-2">
								<h5>주문리스트</h5>
								<button id="editOrder"
									onclick="location.href='/product/prdCode=${prdInfo[0].PRD_CODE}';"
									...
									<tr class="prdItem">
										<td class="shoping__cart__item"><span
											class="row mx-auto px-0"> <span> <img
													src="${pageContext.request.contextPath}/resources/prdImg/${lists.PRD_CODE}.png"
													alt="${lists.PRD_NAME}_image">
											</span> <span class="my-auto"> <b class="mb-1 ORDER_PRDNAME">${lists.PRD_NAME}<small
														class="pl-1 prdCode">${lists.PRD_CODE}</small></b>
													<p class="mb-0">${lists.PRD_BRAND}</p>
													<p class="mb-0">
														<span class="qty-size">${lists.ORDER_PRDSIZE}</span> /
														${lists.PRD_COLOR}
													</p>
											</span>
										</span></td>
										<td class="shoping__cart__price"><c:set var="orgPrice"
												value="${lists.PRD_PRICE}" /> <c:if
												test="${lists.PRD_DISRATE == 0}">
												<c:set var="finalPrice" value="${lists.PRD_PRICE}" />
												<span class="digits orgPrice"
													data-orgPrice="${lists.PRD_PRICE}">${finalPrice}원</span>
											</c:if> <c:if test="${lists.PRD_DISRATE != 0}">
												...
												<span class="mb-0 digits orgPrice discntNum numFont"
													data-orgPrice="${lists.PRD_PRICE}">${lists.PRD_PRICE}원</span>
												<span
													class="mb-0 font-weight-bold digits numFont discntNumCk">${finalPrice}원</span>
											</c:if></td>
										...
										<td class="shoping__cart__total numFont digits"
											data-value="ORDER_PRICE">${finalPrice*lists.ORDER_AMOUNT}원</td>
									</tr>
								</c:forEach>
							</c:if>
						</tbody>
					...
					<div class="row mx-auto px-0">
						<div class="col-lg-8 col-md-6">
							...
							<div class="row borderTop">
								...
								<div class="checkout__input col-lg-6">
									<span><input id="CUS_NAME" name="CUS_NAME" type="text"
										maxlength="49" required="required" value="${cusInfo.CUS_NAME}"
										readonly /></span>
								</div>
							</div>
							<div class="row">
								...
								<div class="checkout__input col-lg-6">
									<span><input id="CUS_TEL" name="CUS_TEL" type="tel"
										maxlength="11" required="required" value="${cusInfo.CUS_TEL}"
										readonly /></span>
								</div>
							</div>
							<div class="row">
								...
								<div class="checkout__input col-lg-6">
									<span><input id="CUS_EMAIL" name="CUS_EMAIL"
										type="email" maxlength="100" required="required"
										value="${cusInfo.CUS_EMAIL}" readonly /></span>
								</div>
							</div>
							...
							<div class="row borderTop">
								...
								<div class="checkout__input col-lg-6">
									...
									</label> <label for="recentlyDeliv" class="radioLabel"> <input
										type="radio" id="recentlyDeliv" name="delivAddr"
										value="recently" /> <span>최근배송지</span>
							...
							<div class="row">
								<label for="ORDER_RECEIVER" class="col-lg-3">이름<span
									class="inpReq">*</span></label>
								<div class="checkout__input col-lg-6">
									<span><input id="ORDER_RECEIVER" class="newDelivInput"
										name="ORDER_RECEIVER" type="text" maxlength="49"
										required="required" value="${cusInfo.CUS_NAME}" readonly /></span>
								</div>
							</div>
							<div class="row">
								<label for="ORDER_TEL" class="col-lg-3">휴대폰 번호<span
									class="inpReq">*</span></label>
								<div class="checkout__input col-lg-6">
									<span><input id="ORDER_TEL" class="newDelivInput"
										name="ORDER_TEL" type="tel" maxlength="11" required="required"
										value="${cusInfo.CUS_TEL}" readonly /></span>
								</div>
							</div>
							<input type="hidden" id="isCart" value="${isCart}" />
							<!-- 기본주소 -->
							<c:set var="getAddr" value="${fn:split(cusInfo.CUS_ADDR, '|')}" />
							<c:set var="delivAddrExtra"
								value="${getAddr[fn:length(getAddr)-1]}" />
							<c:set var="delivAddrJibun"
								value="${getAddr[fn:length(getAddr)-2]}" />
							<c:set var="delivAddrRoad"
								value="${getAddr[fn:length(getAddr)-3]}" />
							...
							<!-- 최근주소 -->
							<c:set var="getAddr2"
								value="${fn:split(cusInfo.CUS_DELIV_ADDR, '|')}" />
							<c:set var="delivAddrExtra2"
								value="${getAddr2[fn:length(getAddr2)-1]}" />
							<c:set var="delivAddrJibun2"
								value="${getAddr2[fn:length(getAddr2)-2]}" />
							<c:set var="delivAddrRoad2"
								value="${getAddr2[fn:length(getAddr2)-3]}" />
							...
							<div class="row">
								...
								<div class="checkout__input col-lg-3">
									<span><input type="text" id="delivAddrZip"
										class="newDelivInput" placeholder="우편번호" title="우편번호"
										required="required" data-zip="${delivAddrZip}"
										value="${delivAddrZip}" readonly /></span>
								</div>
								<button type="button"
									class="col-lg-2 text-center btn btn-secondary"
									onclick="findAddr()">우편번호 찾기</button>
							</div>
							<div class="row">
								<div class="checkout__input col-lg-9 offset-lg-3">
									<span><input type="text" id="delivAddrRoad"
										class="newDelivInput" placeholder="" title="기본주소"
										required="required" data-road="${delivAddrRoad}"
										value="${delivAddrRoad}" readonly /></span>
								</div>
							</div>
							<div class="row">
								<div class="checkout__input col-lg-9 offset-lg-3">
									<span> <input type="hidden" id="delivAddrJibun"
										class="newDelivInput" data-jibun="${delivAddrJibun}"
										value="${delivAddrJibun}" /> <input type="text"
										id="delivAddrExtra" class="newDelivInput"
										data-extra="${delivAddrExtra}" value="${delivAddrExtra}"
										title="상세주소" placeholder="상세주소를 입력해주세요." />
									</span> <span id="guide" style="color: #999; display: none"></span>
								</div>
							</div>
							<div class="row">
								<label for="ORDER_MEMO" class="col-lg-3">배송시 요청사항 </label>
								<div class="checkout__input wrapper col-lg-9">
									<select id="delivMemo" data-display="select" class="mb-2 wide"
										title="배송시 요청사항">
						...
						<div class="col-lg-4 col-md-6">
							<div class="checkout__order">
								<h4>결제정보</h4>
								<ul>
									<li>총 주문 가격<span>원</span><span
										class="totalOrgPrice digits">-</span></li>
									<li>할인<span>원</span><span class="totalDiscnt digits">-</span></li>
									<li class="points">포인트 사용 <input type="number"
										placeholder="0" class="text-right usePoint"
										name="ORDER_USEPOINT"></input><span>P</span>
										<p class="mb-1">
											<small>사용가능한 포인트: <span>P</span><span
												class="avPoint digits hasPoint"
												data-hasPoint="${cusInfo.CUS_POINT}">${cusInfo.CUS_POINT}</span></small>
										</p>
									</li>
									<li>배송비 <span>무료</span></li>
								</ul>
								<div class="checkout__order__total my-2 pb-0 pt-3">
									총 결제금액 <span class="text-right"> <span
										class="totalPriceCon-num float-left digits">-</span> <span>원</span>
									</span>
								</div>
								<small class="float-right mb-4">구매 확정시 <span
									class="avPoint savePoint digits">-</span>P 적립예정
								</small>
								...
								<%}%>
									var isCart = $(
														...
														var ORDER_TEL = $(
																...
																.val();
														var ORDER_MEMO = $(
																"input[name=ORDER_MEMO]")
																.val();
														if (ORDER_MEMO == "") {
															ORDER_MEMO = null;
														}
														var ORDER_RECEIVER = $(
																"input[name=ORDER_RECEIVER]")
																.val();
														var ORDER_USEPOINT = $(
																"input[name=ORDER_USEPOINT]")
																.val();
														if (ORDER_USEPOINT == "")
															ORDER_USEPOINT = 0;
														var ORDER_ADDR = $(
																...
																.request_pay(
																		...
																		function(
																			...
																			if (rsp.success) {
																				...
																				setTimeout(
																						function() {
																							location.href = "/checkout/complete/orderNo="
																									+ ORDER_NUM;
																						},
																						2000);
																			} else {
						...
						function() {
							...
							$('#delivMemo').on(
									...
									function() {
										...
										$("#ORDER_MEMO").val(
												$("#delivMemo option:selected")
														.text());
										if (selected == "write") {
											$("#ORDER_MEMO").val('');
											$("#ORDER_MEMO").removeAttr(
													'readonly');
										}
									});
							...
							$('.orgPrice').each(
									function(index, item) {
										totalPriceNoDiscnt += parseInt($(this)
												.text())
												* parseInt($(this)
														.parents("tr").find(
																".amountNum")
							...
							$('.orgPrice').each(
									function(index, item) {
										if ($(this).siblings("span").hasClass(
												"discntNumCk")) {
											totalPriceCon_num += parseInt($(
													this).siblings(
													$("span.discntNumCk"))
													.text())
							...
							//할인
							var totalDiscnt = 0;
							totalDiscnt = totalPriceNoDiscnt
									- totalPriceCon_num;
							$(".totalDiscnt").text(totalDiscnt);

							//적립예정 init
							$(".savePoint").text(
									parseInt(((totalPriceCon_num * 0.03) / 10),
											10) * 10);
							...
							$(".usePoint")
									.on(
											...
											function() {
												...
												if (parseInt($(".hasPoint")
														.attr("data-hasPoint")) < parseInt(usePoint)) {
