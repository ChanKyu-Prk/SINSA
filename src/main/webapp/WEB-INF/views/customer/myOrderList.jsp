	...
	<div class="container con_top_margin">
		<div class="row">
			...
			<div class="col-9">
				...
				<input type="date" id="date1" value="${date1 }"> <input type="date" id="date2" value="${date2 }">
					...
					<c:choose>
						...
						<c:when test="${date != null}">
							<c:choose>
								...
								<c:when test="${pageInfo.getStartPage()==1}">
									<li class="page-item"><a class="page-link"
										href="myOrderList.do?page=${pageInfo.getStartPage()}&date=${date}"
										tabindex="-1">이전</a></li>
								</c:when>
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="myOrderList.do?page=${pageInfo.getStartPage()-1}&date=${date}"
										tabindex="-1">이전</a></li>
								</c:otherwise>
							</c:choose>
							...
							<c:forEach begin="${pageInfo.getStartPage()}"
								...
								<c:choose>
									...
									<c:otherwise>
										<li class="page-item"><a class="page-link"
											href="myOrderList.do?page=${state.index}&date=${date}">${state.index}</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							...
							<c:choose>
								...
								<c:when test="${pageInfo.getEndPage()==pageInfo.getMaxPage()}">
									<li class="page-item"><a class="page-link"
										href="myOrderList.do?page=${pageInfo.getEndPage()}&date=${date}">다음</a></li>
								</c:when>
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="myOrderList.do?page=${pageInfo.getEndPage()+1}&date=${date}">다음</a></li>
								</c:otherwise>
							</c:choose>
						</c:when>
