					...
					<ul class="pagination">
						<c:choose>
							...
							<c:when test="${pageInfo.getStartPage()==1}">
								<li class="page-item"><a class="page-link"
									href="jjimList.do?page=${pageInfo.getStartPage()}"
									tabindex="-1">이전</a></li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="jjimList.do?page=${pageInfo.getStartPage()-1}"
									tabindex="-1">이전</a></li>
							</c:otherwise>
						</c:choose>
						...
						<c:forEach begin="${pageInfo.getStartPage()}"
							...
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="jjimList.do?page=${state.index}">${state.index}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						...
						<c:choose>
							...
							<c:when test="${pageInfo.getEndPage()==pageInfo.getMaxPage()}">
								<li class="page-item"><a class="page-link"
									href="jjimList.do?page=${pageInfo.getEndPage()}">다음</a></li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="jjimList.do?page=${pageInfo.getEndPage()+1}">다음</a></li>
							</c:otherwise>
						</c:choose>
