...
public class NoticeController {
	...
	public String getNoticeList(Model model, @RequestParam(value = "page", required = false) String pages,
		...
		if (fieldName == "all" || fieldName.equals("all")) {
			listCount = noticeService.notice_all_search_count(map);
			List<NoticeVO> list = noticeService.notice_all_search(map);
			model.addAttribute("noticeList", list);			
		} else {
			listCount = noticeService.notice_list_count(map);
			List<NoticeVO> list = noticeService.notice_list(map);
			model.addAttribute("noticeList", list);
		}
